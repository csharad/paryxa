...
class UserAccountSetting extends Component {
  ...
  render() {
    ...
    const confirmation = (updateCreds, { loading }) => (
      <Dialog
        open={isOpen}
        classes={{
          ...
          onSubmit={async (cred, action) => {
            ...
            try {
              ...
              if (e.graphQLErrors[0].extensions.kind === "NOT_UNIQUE") {
                ...
                e.graphQLErrors[0].extensions.kind === "INCORRECT_PASSWORD"
              ) {
                action.setErrors({
            ...
            <form onSubmit={handleSubmit}>
              <div className={classes.confirmationBody}>
                <DialogContentText variant="body2">
                  Your current password is required to change your credentials.
                </DialogContentText>
    ...
    return (
      <Fragment>
        <AuthenticatedUser>
          {({ data, loading }) => (!loading ? account(data) : null)}
        </AuthenticatedUser>
        <UpdateMyCredentials>{confirmation}</UpdateMyCredentials>
      </Fragment>
    );
  }
}
